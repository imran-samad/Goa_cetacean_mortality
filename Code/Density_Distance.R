...
dol_dat$PDistance_m <- dol_dat$Distance_m *
  sin((abs(dol_dat$Angle) * (pi / 180)))
...
for (i in 1:nrow(dol_dat)) {
  row_loc <- which.min(abs(
    dol_dat$Waypoint_number[i] - dol_cov$Waypoint.number
  ))[1]
  temp_df <- rbind(
    temp_df,
    dol_cov[
...
dist_data <- data.frame(cbind(
  ...
  "FB" = dol_dat$Number.of.fishing.boats..small.medium +
    dol_dat$Number.of.fishing.boats..large,
  "TB" = dol_dat$Number.of.tourist.boats
)) #41 is the transect end, effort is per replicate
dist_data[, 3:ncol(dist_data)] <- sapply(
  dist_data[, 3:ncol(dist_data)],
  as.numeric
)
...
jpeg(
  "Results/2025/Graphs/density_detection_functions.jpg",
  width = 1200,
  height = 1200,
  units = "px"
)
