...
y_strand_data <- array(
  y_strand_data$Count,
  c(length(unique(y_strand_data$Month)), length(unique(y_strand_data$Year)))
)
...
str_prb <- array(
  str_prb$mean_prob,
  c(length(unique(str_prb$month)), length(unique(str_prb$year)))
)
...
op_sum1 <- summary(mcmc.out.iohd)
op_sum1 <- data.frame(
  cbind(op_sum1$statistics[1:12, ], op_sum1$quantiles[1:12, ]),
  'Species' = "Indian Ocean humpback dolphin"
)
op_sum2 <- summary(mcmc.out.ipfp)
op_sum2 <- data.frame(
  cbind(op_sum2$statistics[1:12, ], op_sum2$quantiles[1:12, ]),
  'Species' = "Indo-Pacific finless porpoise"
)
op_sum1 |>
  rbind(op_sum2) |>
...
ggsave(
  "Results/2025/Graphs/Estimated_mortality_monthly_IOHD_IPFP_95CI.jpg",
  width = 6,
  height = 8
)
...
close_pairs_df$cluster <- components$membership[as.character(
  close_pairs_df$id1
)]
