...
This section calculates the mean reviewer score for each case and then derives the percentage difference from the self-reported decomposition and PMI values. This is the most technical part of the report because it translates multiple reviewer assessments into a single summary metric.
...
This is the most computationally intensive section. It matches each boat-survey waypoint to the appropriate grid cell, aggregates counts across all survey events, and then calculates the average fishing and tourist activity per grid cell.
